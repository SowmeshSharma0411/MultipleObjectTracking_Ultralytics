...
def count_nearby_objects(current_object, all_objects, frame_width, frame_height):
    
    if len(all_objects) < 2:
        return 0
    
    distance_threshold = max(frame_width, frame_height) * 0.01
...
def process_trajectory(track_id, track, all_tracks, frame_width, frame_height, vehicle_classes, frame_stride, fps, frame_diagonal):
    min_track_length = 3
    if len(track) >= min_track_length:
        ...
        for i, point in enumerate(track):
            ...
            nearby = count_nearby_objects(point, current_frame_objects, frame_width, frame_height)
...
def process_video(video_path):
    ...
    # Save track data
    with open(os.path.join(output_dir, "track_history.json"), "w") as json_f:
